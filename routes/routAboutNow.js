...
routAboutNow.get("/now:idFilm", async (req, res) => {
  try {
    ...
    for (let i = 0; i < zapRaspisanie.length; i++) {
      if (zapRaspisanie[i].seans.getMonth() == monCurrent && zapRaspisanie[i].seans.getDate() == dateCurrent) {
        obgFortm.idr=(zapRaspisanie[i].idRasp);
        obgFortm.sns=(zapRaspisanie[i].seans.getHours() +":" + zapRaspisanie[i].seans.getMinutes());
        tm.push(obgFortm);
        obgFortm=new Object();
        if (i == zapRaspisanie.length - 1) {
          seans.date = zapRaspisanie[i].seans.getDate();
          seans.month = new Intl.DateTimeFormat("uk-UK", {month: "long",}).format(zapRaspisanie[i].seans);
          seans.weekday = new Intl.DateTimeFormat("uk-UK", {weekday: "long",}).format(zapRaspisanie[i].seans);
          seans.rsp = tm;
          formRaspisanie[formRaspisanieCount] = seans;
        }
      } else {
        seans.date = zapRaspisanie[i - 1].seans.getDate();
        seans.month = new Intl.DateTimeFormat("uk-UK", {month: "long"}).format(zapRaspisanie[i - 1].seans);
        seans.weekday = new Intl.DateTimeFormat("uk-UK", {weekday: "long",}).format(zapRaspisanie[i - 1].seans);
        seans.rsp = tm;
        formRaspisanie[formRaspisanieCount] = seans;
